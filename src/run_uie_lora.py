...
class UIETrainingArguments(Seq2SeqTrainingArguments):
    ...
    method: str = field(
        default="lora", metadata={"help": "the different method name, default is olora"}
    )
    
    # for olora
    lamda_o: float = field(default = 0.5)
    lamda_l2: float = field(default = 0.005)
    
    # for proposed method
    prev_scheduler_path: str = field(default = "")
    a: float = field(default = 0.99)
    b: float = field(default = 0.025) 
    c: float = field(default = 0.15)
...
def main():
    ...
    if training_args.method == "olora" or training_args.method == "inclora":
        from custom_peft import get_peft_config, get_peft_model, LoraConfig, TaskType, PeftModel, PeftConfig # add
    else:
    ...
    if 'adapter' in model_args.model_name_or_path: # add lora-adapter to the original model
        ...
        if hasattr(LoraConfig, "save_lora"):
            peft_config = LoraConfig(
                task_type=TaskType.CAUSAL_LM, inference_mode=False, r=model_args.lora_dim, lora_alpha=32, lora_dropout=0.1, save_loranew=(not model_args.inc)
            )
        else:
            peft_config = LoraConfig(
        ...
        if hasattr(LoraConfig, "save_lora"):
            peft_config = LoraConfig(
                task_type=TaskType.SEQ_2_SEQ_LM, inference_mode=False, r=model_args.lora_dim, lora_alpha=32, lora_dropout=0.1, save_loranew=(not model_args.inc)
            )
        else:
            peft_config = LoraConfig(
                task_type=TaskType.SEQ_2_SEQ_LM, inference_mode=False, r=model_args.lora_dim, lora_alpha=32, lora_dropout=0.1
            )
        # peft_config.target_modules = ["v"]
        model = get_peft_model(model, peft_config)
    ...
    if training_args.do_train:
        ...
        if training_args.method == "srb":
            if os.path.isdir(training_args.output_dir):
                import torch
                new_state_dict = {}
                new_state_dict["hidden_model_state"] = trainer.hidden_model_state
                new_state_dict["prev_model_state"] = trainer.prev_model_state
                torch.save(new_state_dict, os.path.join(training_args.output_dir, f"model_state_{str(trainer.model.device)}.pt"))
                
                trainer._save_optimizer_and_scheduler(training_args.output_dir)
            
                # new_state_dict = trainer.lr_scheduler.scheduler.__dict__
                # torch.save(new_state_dict, os.path.join(training_args.output_dir, f"scheduler_{str(trainer.model.device)}.pt"))
                # new_state_dict = trainer.lr_scheduler.scheduler.optimizer.state_dict()
                # torch.save(new_state_dict, os.path.join(training_args.output_dir, f"optimizer_{str(trainer.model.device)}.pt"))
