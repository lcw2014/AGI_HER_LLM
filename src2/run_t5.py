...
from utils.cl_collator import DataCollator
from utils.cl_dataset import gen_cache_path

from utils.assets import task_config, lora_state_dict_A, lora_state_dict_B

from cl_trainer import DenserEvalCallback, skip_instructions
from utils.compute_metrics import compute_metrics, compute_grouped_metrics
...
def main():
    ...
    raw_datasets = load_dataset(
        os.path.join(CURRENT_DIR, "utils/cl_dataset.py"),
    ...
    if training_args.model_name == 'gainlora_inflora':
        from models.t5_large import T5ForConditionalGeneration
    ...
    elif training_args.model_name == 'sapt':
        from models.t5_large_sapt import T5ForConditionalGeneration
        model = T5ForConditionalGeneration.from_pretrained(
            model_args.model_name_or_path,
            prompt_config,
            from_tf=bool(".ckpt" in model_args.model_name_or_path),
        ...
        from models.t5_large_base import T5ForConditionalGeneration
        model = T5ForConditionalGeneration.from_pretrained(
            model_args.model_name_or_path,
            prompt_config,
            from_tf=bool(".ckpt" in model_args.model_name_or_path),
    ...
    if model_args.load_checkpoint_from:
        ...
        if 'gainlora' in training_args.model_name:
            model.encoder.previous_trans_input.input_linear[0].data.copy_(torch.load(model_args.load_checkpoint_from)['0.weight'])
            model.encoder.previous_trans_input.output_linear[0].data.copy_(torch.load(model_args.load_checkpoint_from)['2.weight'])
            model.encoder.previous_trans_input.state_dict()
            if cur_task_id > 1:
                model.encoder.previous_trans_input.input_linear[1:].data.copy_(torch.load(model_args.load_checkpoint_from.replace('trans_input.pt', 'previous_trans_input.pt'))['input_linear'])
                model.encoder.previous_trans_input.output_linear[1:].data.copy_(torch.load(model_args.load_checkpoint_from.replace('trans_input.pt', 'previous_trans_input.pt'))['output_linear'])
    ...
    for name, param in model.named_parameters():
        if  training_args.model_name in ['gainlora_olora', 'sapt']:
        ...
        elif training_args.model_name in ['gainlora_inflora']:
            param.requires_grad = False
    ...
    if training_args.model_name == 'gainlora_inflora':
        from cl_trainer import Trainer
        trainer = Trainer(
            model=model,
            args=training_args,
        ...
        from cl_trainer_sapt import Trainer
        trainer = Trainer(
            model=model,
            args=training_args,
            train_dataset=train_dataset if training_args.do_train else None,
    ...
    if training_args.do_train:
        ...
        if not prompt_config["run_single"]:
            if prompt_config["previous_prompt_key_path"] is not None and "gainlora" in training_args.model_name:
