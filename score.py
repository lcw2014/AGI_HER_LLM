...
for i in range(len(data_list)):
    ...
    for j in range(i+1):
        if 'order7' in run_name or 'order8' in run_name:
            score=inference_result[f'predict_rougeL_for_{data_list[j]}'] #  "predict_exact_match_for_" for Long Sequence
        else:
...
for i in range(task_num):
    ...
    if 'order7' in run_name or 'order8' in run_name:
        score=inference_result[f'predict_rougeL_for_{single_task_list[i]}']
    else:
